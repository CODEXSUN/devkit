...
The Codex runtime enforces the workspace sandbox and approval mode. DevKit observes tool events
and stops the turn when a run limit is reached. The policy catalog does not yet intercept each tool
before each Codex action. DevKit now owns a separate registered verification and integration gate.
...
runtime. DevKit records command activity but does not approve commands from a project allowlist.
...
DevKit runs verification commands without a shell and stores each attempt. Failed checks can enter
rework and run again. A passed run exposes a separate local commit approval. DevKit never pushes
