...
Every DevKit agent connector uses the same tool vocabulary. A connector can report an action only after the desktop runtime observes it. A model response is not evidence that a file, command, Git operation, or deployment happened.
...
- Codex retains its native App Server tool execution and streams its observed events into DevKit.
- OpenCode runs as a separate packaged CLI worker. DevKit uses its built-in `plan` agent, disables
