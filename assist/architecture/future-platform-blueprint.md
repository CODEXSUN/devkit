# DevKit Future Platform Blueprint
...
This blueprint records the intended evolution of DevKit into a local-first, hybrid developer and
engineering orchestration platform. CodeLogicX remains the external product label; `devkit`
...
- DevKit is a standalone, local-authenticated, single-client modular monolith.
- Platform owns identity, executable API and web servers, composition, and one MariaDB connection.
- DevKit modules own engineering product behavior.
- Browser and API routes remain under `/app/devkit/*` and `/api/devkit/*`.
...
DevKit should become an engineering operating platform where developers, projects, source code,
...
flowchart TB
  ...
  web --> control["DevKit Cloud Control Plane"]
  ...
  desktop --> localRuntime["devkit-node: Docker, Git, Browser, Local Models"]
  vps --> remoteRuntime["devkit-node: Workers and Docker"]
  gpu --> gpuRuntime["devkit-node: Accelerated Models"]
...
| Execution     | Application server and guarded deployment tooling | Distributed `devkit-node` execution network                  |
...
```text
React UI -> Tauri boundary -> DevKit local runtime -> Docker, Git, agents, models, browser, OS
```

Each capable machine runs `devkit-node`, which will:
...
```text
Issue one-time registration token -> Install devkit-node -> Register
...
Create the minimal `devkit-node`, unique node registration, heartbeat, capability reporting,
