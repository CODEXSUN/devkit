...
export function syncTokenHash(token: string) {
  return createHash("sha256")
    .update(`${token}:${requiredSecret("DEVKIT_SYNC_TOKEN_PEPPER")}`)
    .digest("hex");
}
...
export function decryptSyncToken(value: string) {
  ...
  if (version !== "v1" || !ivValue || !tagValue || !ciphertextValue) {
    throw new Error("Stored DevKit sync token is invalid.");
  }
...
function encryptionKey() {
  return createHash("sha256")
    .update(requiredSecret("DEVKIT_SYNC_ENCRYPTION_KEY"))
    .digest();
}
...
function requiredSecret(name: string) {
  ...
  if (!value || value.length < 32) {
    throw new Error(
      `${name} or JWT_SECRET must contain at least 32 characters.`,
    );
  }
  return value;
}
