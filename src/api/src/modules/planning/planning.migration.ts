...
export async function migratePlanningModule(database: Kysely<DevkitDatabase>) {
  ...
  `.execute(database);
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_planning_board_links (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      sync_version INT UNSIGNED NOT NULL DEFAULT 1,
      sync_updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_planning_board_links_uuid (uuid),
      UNIQUE KEY uq_devkit_planning_board_record (board_uuid, record_kind, record_uuid),
      KEY idx_devkit_planning_record (record_kind, record_uuid)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_planning_comments (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      board_uuid CHAR(8) NOT NULL,
      ...
      sync_version INT UNSIGNED NOT NULL DEFAULT 1,
      sync_updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_planning_comments_uuid (uuid),
      KEY idx_devkit_planning_comments_board (board_uuid, status, created_at)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_planning_reactions (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      sync_version INT UNSIGNED NOT NULL DEFAULT 1,
      sync_updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_planning_reactions_uuid (uuid),
      UNIQUE KEY uq_devkit_planning_reaction_actor (comment_uuid, reaction, created_by),
      KEY idx_devkit_planning_reactions_comment (comment_uuid)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);
