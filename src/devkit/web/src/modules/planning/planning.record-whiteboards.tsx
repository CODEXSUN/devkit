      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        {recent ? (
          <button
            ...
            type="button"
            onClick={() =>
              window.location.assign(`/app/devkit/planning/${recent.uuid}`)
            }
          >
            Open recent
          </button>
...
export function openRecordWhiteboards(record: ProjectManagerRecord) {
  window.location.assign(
    `/app/devkit/planning?recordKind=${encodeURIComponent(record.kind)}&recordUuid=${encodeURIComponent(record.id)}`,
  );
}
