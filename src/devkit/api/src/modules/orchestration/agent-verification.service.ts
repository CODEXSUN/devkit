...
function verificationCommands() {
  const configured = process.env.DEVKIT_AGENT_VERIFICATION_COMMANDS?.trim();
  if (!configured) return builtInCommands;
  let value: unknown;
  try {
    value = JSON.parse(configured);
  } catch {
    throw AppError.validation("DEVKIT_AGENT_VERIFICATION_COMMANDS must contain a JSON array.");
  }
