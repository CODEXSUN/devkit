...
const sourceDir = resolveDevkitSeedDirectory(
  import.meta.url,
...
async function seedPlatforms(database: Kysely<DevkitDatabase>) {
  if ((await count(database, "devkit_project_manager_registry_platforms")) > 0)
    return 0;
...
async function seedGroups(database: Kysely<DevkitDatabase>) {
  if ((await count(database, "devkit_project_manager_registry_groups")) > 0)
    return 0;
  const rows =
...
async function seedModules(database: Kysely<DevkitDatabase>) {
  if ((await count(database, "devkit_project_manager_registry_modules")) > 0)
    return 0;
