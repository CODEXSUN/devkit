...
RUNTIME_ENV="${DEVKIT_RUNTIME_ENV:-$ROOT_DIR/.env}"
...
Interactive standalone DevKit container installation.
...
Configuration:
  .env                    DevKit runtime and application settings
...
Included:
  ...
  - Repository-owned UI
  - DevKit Platform API and Web
  - Reused existing Docker network and MariaDB, or DevKit-owned infrastructure

Excluded:
  - CXApp, Billing, Mail, TMApp, Redis, Media, and all other stacks

The setup builds only this self-contained DevKit repository. It asks whether
to reuse an existing Docker network and running MariaDB container or create a
dedicated DevKit network and MariaDB. DevKit uses only MariaDB and does not
inspect or depend on Redis, Media, or another application stack.
If dedicated DevKit MariaDB data already exists, setup separately asks
whether to reuse or freshly recreate only the DevKit database volume.
EOF
}
...
prepare_deploy_environment() {
  ...
  case "$(file_value "$DEPLOY_ENV" DB_USER)" in
    ""|root)
      set_file_value "$DEPLOY_ENV" DB_USER devkit
      echo "Configured the dedicated deployment database user: devkit"
      ;;
  esac
  set_default_if_empty "$DEPLOY_ENV" DB_NAME devkit_db
  ...
  set_file_value "$DEPLOY_ENV" DEVKIT_VERSION "$version"
  set_file_value "$DEPLOY_ENV" DEVKIT_IMAGE_TAG "$version"
...
configure_deploy_environment() {
  ...
  prompt_setting "$DEPLOY_ENV" DEVKIT_COMPOSE_PROJECT "Compose project" devkit
  prompt_setting "$DEPLOY_ENV" DEVKIT_IMAGE_REGISTRY "Image registry/prefix" devkit
...
prepare_runtime_environment() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    database_host="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  else
...
select_infrastructure_mode() {
  ...
  while true; do
    read -r -p \
      "Reuse CXApp network and MariaDB, or create dedicated DevKit infrastructure? [reuse/dedicated] " \
      answer
...
prepare_shared_infrastructure() {
  ...
  mariadb="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  network="$(file_value "$DEPLOY_ENV" DEVKIT_NETWORK cxapp-network)"
  shared_env="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_ENV_FILE)"
  ...
  if [[ -n "$shared_env" ]]; then
    ...
    set_file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_ROOT_PASSWORD "$root_password"
  ...
  root_password="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_ROOT_PASSWORD)"
  [[ -n "$root_password" ]] || {
    echo "Shared MariaDB root password is unavailable." >&2
    echo "Configure DEVKIT_SHARED_MARIADB_ROOT_PASSWORD in $DEPLOY_ENV." >&2
    exit 78
  }
}
...
configure_shared_infrastructure() {
  ...
  prompt_setting "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_CONTAINER_NAME \
    "CXApp MariaDB container name" cxapp-mariadb
  prompt_setting "$DEPLOY_ENV" DEVKIT_NETWORK "CXApp Docker network" cxapp-network
  set_file_value "$DEPLOY_ENV" DEVKIT_NETWORK_EXTERNAL true
  prompt_setting "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_ENV_FILE \
...
configure_dedicated_infrastructure() {
  ...
  echo
  echo "Dedicated DevKit infrastructure settings"
  if [[ "$(file_value "$DEPLOY_ENV" DEVKIT_NETWORK_EXTERNAL false)" == true ]]; then
    set_file_value "$DEPLOY_ENV" DEVKIT_NETWORK devkit-network
  fi
  set_file_value "$DEPLOY_ENV" DEVKIT_NETWORK_EXTERNAL false
  prompt_setting "$DEPLOY_ENV" DEVKIT_MARIADB_CONTAINER_NAME \
    "Dedicated MariaDB container name" devkit-mariadb
  container="$(file_value "$DEPLOY_ENV" DEVKIT_MARIADB_CONTAINER_NAME devkit-mariadb)"
  while docker container inspect "$container" >/dev/null 2>&1 &&
    ! container_is_compose_service "$container" \
      "$(file_value "$DEPLOY_ENV" DEVKIT_COMPOSE_PROJECT devkit)" mariadb; do
    ...
    set_file_value "$DEPLOY_ENV" DEVKIT_MARIADB_CONTAINER_NAME "$replacement"
    container="$replacement"
  done
  prompt_setting "$DEPLOY_ENV" DEVKIT_NETWORK "Dedicated Docker network" devkit-network
  prompt_setting "$DEPLOY_ENV" DEVKIT_MARIADB_DATA_VOLUME \
    "MariaDB data volume" devkit-mariadb-data
...
compose() {
  DEVKIT_RUNTIME_ENV_FILE="$RUNTIME_ENV" docker compose \
...
validate_deploy_environment() {
  ...
  ensure_available_host_port \
    DEVKIT_API_HOST_PORT \
    "API host port" \
    "$(file_value "$DEPLOY_ENV" DEVKIT_API_CONTAINER_NAME devkit-api)"
  ensure_available_host_port \
    DEVKIT_WEB_HOST_PORT \
    "Web host port" \
    "$(file_value "$DEPLOY_ENV" DEVKIT_WEB_CONTAINER_NAME devkit-web)"
}
...
select_database_mode() {
  ...
  while true; do
    read -r -p "Reuse or freshly recreate DevKit MariaDB data? [reuse/fresh] " answer
    case "${answer:-reuse}" in
      reuse|Reuse|REUSE)
        printf 'reuse'
        return
        ;;
      fresh|Fresh|FRESH)
        read -r -p "Type FRESH $volume to delete only this DevKit volume: " confirmation
...
reconcile_database_user() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    container="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
    root_password="$(file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_ROOT_PASSWORD)"
  else
    container="$(file_value "$DEPLOY_ENV" DEVKIT_MARIADB_CONTAINER_NAME devkit-mariadb)"
...
SQL
  then
    echo "Could not reconcile the DevKit MariaDB application user." >&2
...
if [[ "$infrastructure_mode" == dedicated ]]; then
  database_volume="$(file_value "$DEPLOY_ENV" DEVKIT_MARIADB_DATA_VOLUME devkit-mariadb-data)"
  safe_docker_name "$database_volume"
  database_mode="$(select_database_mode "$database_volume")"
  infrastructure_label="dedicated DevKit network and MariaDB"
else
  database_volume=""
...
echo
echo "Standalone DevKit deployment plan"
echo "  Runtime env: $RUNTIME_ENV"
echo "  Deploy env: $DEPLOY_ENV"
echo "  Source: self-contained DevKit monorepo"
