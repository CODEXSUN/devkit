...
function Test-LocalAi([int]$OllamaPort, [int]$QdrantPort, [string]$Model, [switch]$KeepProbe) {
  ...
  $probeCollection = "devkit_setup_probe"
  ...
  Write-Host "Qdrant health and collection API succeeded$(if ($KeepProbe) { '; retained devkit_setup_probe.' } else { '; temporary probe removed.' })" -ForegroundColor Green
  ...
  $reply = Invoke-RestMethod -Method Post -Uri "$ollamaUrl/api/generate" -ContentType "application/json" -TimeoutSec 90 -Body (@{
      model = $Model
      prompt = "Reply with exactly: DevKit local AI ready"
      stream = $false
    } | ConvertTo-Json)
