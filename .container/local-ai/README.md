# DevKit local AI environment

This operator kit creates a local-only Ollama and Qdrant environment when you explicitly apply it.
It does not start services, download models, alter DevKit credentials, or change the default Codex
...
| DevKit projects, identity, runs, and metadata | MariaDB | Existing transactional product store. |

MariaDB vector search is not added by this kit. Qdrant is a better local vector engine for the
future retrieval use case; MariaDB remains the source of truth for DevKit records. The desktop
...
Qdrant is deliberately provisioned but not used by DevKit yet. Add retrieval only after a measured
...
- The `test` command creates and removes only the `devkit_setup_probe` collection unless you pass
  `-KeepProbe`.
