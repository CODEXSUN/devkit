...
validate_release_contract() {
  ...
  for key in DEVKIT_VERSION DEVKIT_IMAGE_TAG; do
  ...
  configured="$(file_value "$DEPLOY_ENV" DEVKIT_MIGRATION_COMPATIBLE_VERSION)"
  [[ "$configured" == "$source_version" ]] || {
    echo "Migration compatibility is not approved for DevKit $source_version." >&2
    echo "After confirming backward compatibility with the current image, set:" >&2
    echo "  DEVKIT_MIGRATION_COMPATIBLE_VERSION=$source_version" >&2
...
inspect_source_state() {
  ...
  git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
    echo "DevKit update source is not a Git worktree: $ROOT_DIR" >&2
    exit 78
  }
  source_commit="$(git -C "$ROOT_DIR" rev-parse HEAD)"
  if [[ -n "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]]; then
    source_dirty=true
    echo "WARNING: the DevKit Git worktree contains uncommitted or untracked files." >&2
  ...
  cat >"${metadata_file}.partial" <<EOF
{
  ...
  "migrationCompatibilityVersion": "$(file_value "$DEPLOY_ENV" DEVKIT_MIGRATION_COMPATIBLE_VERSION)",
...
project="$(file_value "$DEPLOY_ENV" DEVKIT_COMPOSE_PROJECT devkit)"
api_container="$(file_value "$DEPLOY_ENV" DEVKIT_API_CONTAINER_NAME devkit-api)"
web_container="$(file_value "$DEPLOY_ENV" DEVKIT_WEB_CONTAINER_NAME devkit-web)"
network="$(file_value "$DEPLOY_ENV" DEVKIT_NETWORK cxapp-network)"
network_external="$(file_value "$DEPLOY_ENV" DEVKIT_NETWORK_EXTERNAL false)"
image_registry="$(file_value "$DEPLOY_ENV" DEVKIT_IMAGE_REGISTRY devkit)"
image_tag="$(file_value "$DEPLOY_ENV" DEVKIT_IMAGE_TAG local)"
backup_retention="$(file_value "$DEPLOY_ENV" DEVKIT_BACKUP_RETENTION 10)"
...
minimum_backup_mb="$(file_value "$DEPLOY_ENV" DEVKIT_UPDATE_MIN_BACKUP_FREE_MB 1024)"
minimum_docker_mb="$(file_value "$DEPLOY_ENV" DEVKIT_UPDATE_MIN_DOCKER_FREE_MB 5120)"
...
for setting in \
  "DEVKIT_BACKUP_RETENTION:$backup_retention" \
  "DEVKIT_UPDATE_MIN_BACKUP_FREE_MB:$minimum_backup_mb" \
  "DEVKIT_UPDATE_MIN_DOCKER_FREE_MB:$minimum_docker_mb"; do
  key="${setting%%:*}"
  value="${setting#*:}"
...
for service_spec in "$api_container:api" "$web_container:web"; do
  ...
  docker container inspect "$container" >/dev/null 2>&1 || {
    echo "Existing DevKit $service container was not found: $container" >&2
...
if [[ "$network_external" == true ]]; then
  mariadb_container="$(
    file_value "$DEPLOY_ENV" DEVKIT_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb
  )"
  ...
  mariadb_container="$(
    file_value "$DEPLOY_ENV" DEVKIT_MARIADB_CONTAINER_NAME devkit-mariadb
  )"
  safe_docker_name "$mariadb_container"
  docker container inspect "$mariadb_container" >/dev/null 2>&1 || {
    echo "Existing DevKit MariaDB container was not found: $mariadb_container" >&2
    exit 69
  }
...
for container in "$mariadb_container" "$api_container" "$web_container"; do
  container_is_running "$container" || {
    echo "Existing DevKit container is not running: $container" >&2
    exit 69
  }
done
...
if [[ "$CHECK_ONLY" == true ]]; then
  echo
  echo "Existing DevKit Docker deployment is ready to update."
  exit 0
fi

if [[ "$ASSUME_YES" != true ]]; then
  read -r -p "Build and update the existing DevKit containers? [Y/n] " confirmation
