...
REMOTE="${DEVKIT_WATCHER_REMOTE:-origin}"
BRANCH="${DEVKIT_WATCHER_BRANCH:-main}"
DEPLOY_ENV="${DEVKIT_WATCHER_DEPLOY_ENV:-$REPO_DIR/.container/deploy.env}"
STATE_DIR="${DEVKIT_WATCHER_STATE_DIR:-/var/lib/devkit-update-watcher}"
LOCK_FILE="${DEVKIT_WATCHER_LOCK_FILE:-/run/lock/devkit-update-watcher.lock}"
CHECK_ONLY=false

usage() {
  cat <<'EOF'
Usage: devkit-update-watcher.sh [--check]

Fetch and safely deploy a fast-forward update from origin/main. --check validates
the watcher, repository, Docker runtime, and current DevKit deployment without
...
candidate_dir="$(mktemp -d /var/tmp/devkit-update-candidate.XXXXXX)"
cleanup_candidate() {
...
project="$(sed -n 's/^DEVKIT_COMPOSE_PROJECT=//p' "$DEPLOY_ENV" | tail -n 1)"
image_registry="$(sed -n 's/^DEVKIT_IMAGE_REGISTRY=//p' "$DEPLOY_ENV" | tail -n 1)"
project="${project:-devkit}"
image_registry="${image_registry:-devkit}"
...
log "Deployment completed for DevKit $version at $target_commit"
