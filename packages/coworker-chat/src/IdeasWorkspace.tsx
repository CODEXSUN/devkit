...
export function IdeasWorkspace({ client }: { client: CoworkerClient }) {
  ...
  const visible = useMemo(() => ideas
    .filter((idea) => filter === "all" || ideaCategory(idea) === filter)
  ...
  useEffect(() => {
    void client.projectRecords("discussion").then((records) => setIdeas(records.filter(isIdeaRecord))).catch(() => setIdeas([]));
  }, [client]);

  if (editing) {
    return <ProjectIdeaEditor client={client} idea={editing} onBack={() => setEditing(null)} onSaved={(saved) => {
      if (!isIdeaRecord(saved)) return;
  ...
  return <section className="ideas-workspace">
    ...
    {visible.length ? <div className="ideas-list">{visible.map((idea, index) => <article key={idea.id}><div className="ideas-row-body"><button className="ideas-row-open" onClick={() => setEditing(idea)} type="button"><span><Lightbulb size={18} /></span><b className="ideas-serial">{String(index + 1).padStart(3, "0")}</b><div><strong>{idea.title}</strong><p>{ideaPreview(idea.description)}</p></div></button><div className="ideas-row-details">{idea.referenceId ? <LockKeyhole aria-label="Private to project" size={13} /> : <Globe2 aria-label="Global idea" size={13} />}<button onClick={() => { setFilter(ideaCategory(idea)); setTagFilter(""); }} type="button">{label(ideaCategory(idea))}</button><IdeaTags onSelect={setTagFilter} tags={hashtags(idea.description)} /></div></div><div className="ideas-row-meta"><div className="ideas-status-actions"><em className={`idea-status ${statusTone(idea.status)}`}>{label(idea.status || "open")}</em><button aria-expanded={menuId === idea.id} aria-label={`More actions for ${idea.title}`} className="ideas-more-button" onClick={() => setMenuId((current) => current === idea.id ? null : idea.id)} type="button"><MoreVertical size={17} /></button></div><small><Clock3 size={12} /> {formatUpdated(idea.updatedAt ?? idea.createdAt)}</small>{menuId === idea.id ? <div className="ideas-row-menu"><button onClick={() => setEditing(idea)} type="button"><Info size={14} /> Open details</button>{idea.referenceId ? <button onClick={() => void makePublic(idea)} type="button"><Globe2 size={14} /> Make public</button> : null}<button onClick={() => void archiveIdea(idea)} type="button"><Archive size={14} /> Archive</button><button className="danger" onClick={() => void deleteIdea(idea)} type="button"><Trash2 size={14} /> Delete</button></div> : null}</div></article>)}</div> : <div className="ideas-empty"><Lightbulb size={24} /><strong>No matching ideas</strong><p>Try a different search, filter, or create a new discussion.</p></div>}
  </section>;
}

function isIdeaRecord(record: CoworkerProjectRecord) { return record.kind === "discussion" && record.type === "idea"; }
function ideaCategory(record: CoworkerProjectRecord) { return record.moduleKey || "general"; }
...
function statusTone(status?: string) { return `status-${status || "open"}`; }
