...
const itemParamsSchema = z
  .object({ id: z.string().min(1), kind: kindSchema })
  .strict();
...
const registrySaveSchema = z
  .object({
    ...
    description: z.string().optional(),
    documentation: z
      .record(z.string(), z.array(documentationRowSchema))
      .optional(),
...
export async function registerProjectManagerRoutes(app: FastifyInstance) {
  ...
  app.post("/admin/project-manager/registry/platforms", async (request) =>
    ok(
      await service.createRegistryPlatform(
        registrySaveSchema.parse(request.body),
        actor(request),
      ),
      {
  ...
  app.post("/admin/project-manager/registry/groups", async (request) =>
    ok(
      await service.createRegistryGroup(
        registrySaveSchema.parse(request.body),
        actor(request),
      ),
      {
  ...
  app.post("/admin/project-manager/registry/modules", async (request) =>
    ok(
      await service.createRegistryModule(
        registrySaveSchema.parse(request.body),
        actor(request),
      ),
      {
  ...
  app.delete(
    "/admin/project-manager/:kind/:id/attachments/:attachmentId",
    async (request) => {
      const params = attachmentParamsSchema.parse(request.params);
      return ok(
  ...
  app.post("/admin/project-manager/:kind/:id/deactivate", async (request) => {
    const params = itemParamsSchema.parse(request.params);
    return ok(
      await service.deactivate(params.kind, params.id, actor(request)),
      {
        requestId: request.id,
      },
    );
