...
export class ProjectManagerRepository {
  ...
  async delete(record: ProjectManagerRecord, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .deleteFrom("project_manager_items")
        .where("kind", "=", record.kind)
        .where("uuid", "=", record.id)
        .executeTakeFirstOrThrow();
      await writeActivity(transaction, {
  ...
  async listRegistryPlatforms() {
    const rows = await this.database
      .selectFrom("project_manager_registry_platforms")
      .selectAll()
      .orderBy("sort_order")
  ...
  async listRegistryGroups() {
    const rows = await this.database
      .selectFrom("project_manager_registry_groups")
      .selectAll()
      .orderBy("sort_order")
      .orderBy("updated_at", "desc")
      .execute();
  ...
  async listRegistryModules() {
    const rows = await this.database
      .selectFrom("project_manager_registry_modules")
      .selectAll()
      .orderBy("sort_order")
      .orderBy("updated_at", "desc")
      .execute();
