...
- `launch-desk.services.ts` calls the DevKit orchestration API.
...
3. Optionally set `DEVKIT_CODEX_HOME` to choose the private authentication location.
4. Run `npm.cmd install` from the repository root.
5. Run `npm.cmd run dev`.
6. Open `/app/devkit/launch-desk`.

DevKit defaults to `%LOCALAPPDATA%/CodeLogicX/DevKit/codex`. Keep this directory outside source control. The connection page does not need `OPENAI_API_KEY` because it authenticates the local Codex runtime through the official device flow.
...
- [ ] DevKit uses an authentication location independent from Codex IDE.
