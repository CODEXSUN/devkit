...
export function WorkAutomationWorkspace({
  ...
  useEffect(() => {
    ...
    if (reviewParentId) {
      ...
      if (targetActivity && openedRecord.current !== targetKey) {
        ...
        setStatusFilter("all");
        setPage(1);
        window.history.replaceState({ page: "projects" }, "", "/app/devkit/projects");
      }
      return;
    }
    ...
    if (drillProjectId) {
      ...
      if (targetProject && openedRecord.current !== targetKey) {
        ...
        window.history.replaceState(
          ...
          "",
          `/app/devkit/projects?project=${encodeURIComponent(targetProject.id)}`
        );
      }
      return;
    }
  ...
  function openIssueDelivery(view: "gantt" | "roadmap") {
    ...
    window.location.assign(
      `/app/devkit/roadmap?issue=${encodeURIComponent(issue.id)}${
  ...
  function backToReviews() {
    ...
    if (reviewParentId) {
      window.location.assign(
        `/app/devkit/projects?reviewParent=${encodeURIComponent(reviewParentId)}`
      );
      return;
    }
    window.history.back();
  ...
  function selectRoadmapView(stage: DeliveryStage) {
    if (stage === "roadmap" || stage === "gantt") {
      ...
      if (roadmapIssue) {
        ...
        window.history.replaceState(
          ...
          "",
          `/app/devkit/roadmap?issue=${encodeURIComponent(roadmapIssue.id)}${
            reviewParentId ? `&reviewParent=${encodeURIComponent(reviewParentId)}` : ""
          }${view === "gantt" ? "&view=gantt" : ""}`
        );
      }
    ...
    if (roadmapIssue && stage === "issue") {
      window.location.assign(`/app/devkit/projects?issue=${encodeURIComponent(roadmapIssue.id)}`);
    }
  }
        ...
        <ProjectDashboard
          ...
          onOpenIssues={() => {
            ...
            window.history.replaceState(
              ...
              "",
              `/app/devkit/projects?project=${encodeURIComponent(dashboardProject.id)}&view=issues`
            );
          }}
        />
      ) : (
              ...
              <ProjectCardList
                ...
                onOpen={(record) => {
                  ...
                  window.history.pushState(
                    ...
                    "",
                    `/app/devkit/projects?project=${encodeURIComponent(record.id)}`
                  );
                }}
