...
export function AgentIdeTaskGraph({ run }: { run: AgentRunDetail }) {
  ...
  const personasQuery = useQuery({ queryFn: listAgentPersonas, queryKey: ["devkit", "agent-personas"] });
  const refresh = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey }),
      queryClient.invalidateQueries({ queryKey: ["devkit", "agent-personas"] }),
      queryClient.invalidateQueries({ queryKey: ["devkit", "agent-run", run.uuid] })
    ]);
  };
...
function starterTasks(run: AgentRunDetail, personas: AgentPersona[], supervisor: AgentPersona | null) {
  ...
  return [
    ...
    { agentProfile: "coding", delegatePersonaUuid: find("forge"), dependsOn: ["inspect"], key: "backend", objective: "Implement the module-owned API and persistence slice.", scopePaths: ["apps/devkit/api/"], title: "Build backend slice" },
    { agentProfile: "coding", delegatePersonaUuid: find("canvas"), dependsOn: ["inspect"], key: "frontend", objective: "Implement the Project Agent visual workflow.", scopePaths: ["apps/devkit/web/"], title: "Build frontend slice" },
    { agentProfile: "review", delegatePersonaUuid: supervisor?.uuid ?? null, dependsOn: ["backend", "frontend"], key: "review", objective: "Review delegate evidence, run focused checks, and report integration risks.", scopePaths: ["apps/devkit/", "test/", "tools/e2e/"], title: "Supervisor verification" }
  ];
}
