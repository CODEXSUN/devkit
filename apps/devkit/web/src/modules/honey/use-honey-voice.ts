...
export function useHoneyVoice(onTranscript: (value: string) => void, onComplete: (value: string) => void) {
  ...
  const [silenceTimeout, setSilenceTimeoutState] = useState(() => Number(window.localStorage.getItem("devkit.honey.voice-silence-ms")) || 1800);
  ...
  function setSilenceTimeout(value: number) { setSilenceTimeoutState(value); window.localStorage.setItem("devkit.honey.voice-silence-ms", String(value)); }
