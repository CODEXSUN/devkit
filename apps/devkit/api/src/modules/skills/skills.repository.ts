...
export class SkillsRepository {
  ...
  private async readMetadata(name: string): Promise<SkillMetadata> {
    try {
      return JSON.parse(await readFile(join(this.skillDirectory(name), ".devkit.json"), "utf8")) as SkillMetadata;
    } catch {
  ...
  private async writeMetadata(name: string, metadata: SkillMetadata) {
    requireName(name);
    await writeFile(join(this.skillDirectory(name), ".devkit.json"), JSON.stringify(metadata, null, 2), "utf8");
  }
  ...
  private safeFile(name: string, file: string) {
    ...
    if (!target.startsWith(`${directory}${sep}`) || baseName === ".devkit.json" || baseName === "skill.md") {
...
async function listFiles(directory: string, root = directory): Promise<string[]> {
  ...
  for (const entry of entries) {
    if (entry.name === ".devkit.json" || entry.name.toLowerCase() === "skill.md") continue;
...
function resolveSkillsRoot() {
  return resolve(process.env.DEVKIT_SKILLS_ROOT?.trim() || join(process.cwd(), "assist", "skills", "library"));
}
