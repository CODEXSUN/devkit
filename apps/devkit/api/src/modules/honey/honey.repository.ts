...
class HoneyRepository {
  ...
  async find(threadUuid: string, actorId: string) {
    const thread = await getDevkitDatabase()
      .selectFrom("devkit_honey_threads")
      .selectAll()
  ...
  async approvedMemory(actorId: string) {
    return getDevkitDatabase()
      .selectFrom("devkit_honey_memory")
      .select(["kind", "content"])
      .where("actor_id", "=", actorId)
  ...
  async reviewMemory(
    ...
    const result = await getDevkitDatabase()
      .updateTable("devkit_honey_memory")
  ...
  async rememberCandidate(actorId: string, threadUuid: string, content: string) {
    if (
      ...
      return;
    const existing = await getDevkitDatabase()
      .selectFrom("devkit_honey_memory")
      .select("uuid")
      .where("actor_id", "=", actorId)
    ...
    if (existing) return;
    await getDevkitDatabase()
      .insertInto("devkit_honey_memory")
      .values({
        actor_id: actorId,
        content,
