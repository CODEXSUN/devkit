...
export class PlanningRepository {
  ...
  async list(record?: { kind: PlanningRecordKind; uuid: string }) {
    ...
    if (record)
      query = query.where("uuid", "in", (builder) =>
        builder
          .selectFrom("devkit_planning_board_links")
          .select("board_uuid")
  ...
  async link(boardUuid: string, kind: PlanningRecordKind, recordUuid: string, actor: string) {
    ...
    await this.database
      .insertInto("devkit_planning_board_links")
      .values({
  ...
  async comments(boardUuid: string) {
    ...
    const rows = await this.database
      .selectFrom("devkit_planning_comments")
      .selectAll()
  ...
  async toggleReaction(commentUuid: string, reaction: string, actor: string) {
    ...
    if (existing)
      await this.database
        .updateTable("devkit_planning_reactions")
        .set({
      ...
      await this.database
        .insertInto("devkit_planning_reactions")
        .values({
  ...
  async delete(uuid: string, actor: string) {
    ...
    await this.database
      ...
      .executeTakeFirst();
    await this.database
      .updateTable("devkit_planning_board_links")
      .set({
        sync_status: "deleted",
        sync_updated_at: new Date(),
        sync_version: (eb) => eb("sync_version", "+", 1)
