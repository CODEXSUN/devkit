...
export class NotificationRepository {
  ...
  async create(actorId: string, input: CreateNotificationInput) {
    return this.database.transaction().execute(async (transaction) => {
      const duplicate = await transaction
        .selectFrom("devkit_notification_jobs")
      ...
      await transaction
        .insertInto("devkit_notifications")
        .values({
      ...
      for (const channel of [...new Set(input.channels)]) {
        ...
        await transaction
          .insertInto("devkit_notification_jobs")
          .values({
            backend: "database",
            channel,
            ...
            queue_name: "devkit-notifications",
            status: "pending",
            uuid: jobUuid
          })
          .executeTakeFirstOrThrow();
  ...
  list(recipientActorId: string) {
    return this.database
      .selectFrom("devkit_notifications")
      .selectAll()
  ...
  async markRead(uuid: string, recipientActorId: string) {
    const result = await this.database
      .updateTable("devkit_notifications")
  ...
  findJob(uuid: string) {
    return this.database
      .selectFrom("devkit_notification_jobs")
      .innerJoin(
        "devkit_notifications",
        "devkit_notifications.uuid",
        "devkit_notification_jobs.notification_uuid"
      )
      .select([
        "devkit_notification_jobs.uuid as job_uuid",
        "devkit_notification_jobs.channel",
        "devkit_notification_jobs.attempts",
        "devkit_notification_jobs.max_attempts",
        "devkit_notifications.uuid as notification_uuid",
        "devkit_notifications.recipient_actor_id",
        "devkit_notifications.recipient_email",
        "devkit_notifications.title",
        "devkit_notifications.body",
        "devkit_notifications.category",
        "devkit_notifications.created_at"
      ])
      .where("devkit_notification_jobs.uuid", "=", uuid)
      .executeTakeFirst();
  }

  async claimJob(uuid: string, backend: string) {
    const result = await this.database
      .updateTable("devkit_notification_jobs")
      .set({
