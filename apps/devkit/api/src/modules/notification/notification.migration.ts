...
export const notificationMigration = {
  ...
  key: "devkit.notification.sql.v1"
} as const;

export async function migrateNotificationModule(database: Kysely<DevkitDatabase>) {
  await sql`CREATE TABLE IF NOT EXISTS devkit_notifications (
    ...
    UNIQUE KEY uq_devkit_notifications_uuid (uuid),
    KEY idx_devkit_notifications_recipient (recipient_actor_id, status, created_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`.execute(database);
  await sql`CREATE TABLE IF NOT EXISTS devkit_notification_jobs (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid CHAR(32) NOT NULL,
    notification_uuid CHAR(32) NOT NULL, channel VARCHAR(24) NOT NULL,
    queue_name VARCHAR(80) NOT NULL DEFAULT 'devkit-notifications',
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uq_devkit_notification_jobs_uuid (uuid),
    UNIQUE KEY uq_devkit_notification_jobs_idempotency (idempotency_key),
    KEY idx_devkit_notification_jobs_dispatch (status, available_at, created_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`.execute(database);
