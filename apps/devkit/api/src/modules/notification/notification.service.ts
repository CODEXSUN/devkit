...
export class NotificationService {
  private readonly repository = new NotificationRepository(getDevkitDatabase());

  async create(input: CreateNotificationInput) {
    const result = await this.repository.create(requireDevkitActor().id, input);
    await enqueueNotificationJobs(result.jobIds);
    return toNotification(result.notification);
  }

  async inbox() {
    const rows = await this.repository.list(requireDevkitActor().id);
    return rows.map(toNotification);
  }

  async markRead(uuid: string) {
    return toNotification(await this.repository.markRead(uuid, requireDevkitActor().id));
  }
