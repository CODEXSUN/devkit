...
export class AgentPersonaRepository {
  ...
  async list(actorId: string) {
    const rows = await this.database.selectFrom("devkit_agent_personas").selectAll()
  ...
  async create(input: AgentPersonaInput, actorId: string) {
    const uuid = id();
    await this.database.insertInto("devkit_agent_personas").values({
      actor_id: actorId,
  ...
  async update(uuid: string, input: AgentPersonaInput, actorId: string) {
    ...
    await this.database.updateTable("devkit_agent_personas").set({
      agent_profile: input.agentProfile,
      description: input.description,
      instructions: input.instructions,
      name: input.name,
  ...
  async require(uuid: string, actorId: string) {
    const row = await this.database.selectFrom("devkit_agent_personas").selectAll()
