...
export class AgentTaskGraphRepository {
  ...
  async replace(parentRunUuid: string, actorId: string, input: AgentDecompositionInput) {
    ...
    await this.database.transaction().execute(async (transaction) => {
      ...
      if (dependencies.length) {
        await transaction
          .insertInto("devkit_agent_task_dependencies")
          .values(dependencies)
          .execute();
      }
      await transaction
  ...
  async start(taskUuid: string, actorId: string) {
    ...
    await this.database
      .updateTable("devkit_agent_tasks")
      .set({
  ...
  async executionContext(taskUuid: string, actorId: string) {
    ...
    const [child, parent, persona, dependencies] = await Promise.all([
      ...
      this.database
        .selectFrom("devkit_agent_task_dependencies as dependency")
        .innerJoin(
          "devkit_agent_tasks as required",
          "required.uuid",
  ...
  async review(
    ...
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("devkit_agent_parent_reviews")
        .values({
          actor_id: actorId,
          decision,
          note,
  ...
  async find(parentRunUuid: string, actorId: string) {
    ...
    const [parent, tasks, dependencies, reviews, personas, approvals] = await Promise.all([
      ...
      this.database
        ...
        .execute(),
      this.database
        .selectFrom("devkit_agent_parent_reviews")
        .selectAll()
        .where("parent_run_uuid", "=", parentRunUuid)
        .orderBy("created_at", "desc")
        .execute(),
      this.database
        .selectFrom("devkit_agent_personas")
        .selectAll()
        .where("actor_id", "=", actorId)
        .where("status", "=", "active")
        .execute(),
      this.database
        .selectFrom("devkit_agent_approvals as approval")
