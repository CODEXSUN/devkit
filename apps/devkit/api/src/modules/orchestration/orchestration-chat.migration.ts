...
export async function migrateOrchestrationChat(database: Kysely<DevkitDatabase>) {
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_orchestration_chat_threads (
      ...
      KEY idx_devkit_chat_threads_actor (actor_id, updated_at),
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_orchestration_chat_messages (
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_chat_messages_uuid (uuid),
      KEY idx_devkit_chat_messages_thread (actor_id, thread_uuid, created_at),
      CONSTRAINT fk_devkit_chat_messages_thread
        FOREIGN KEY (thread_uuid) REFERENCES devkit_orchestration_chat_threads (uuid)
        ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);
  await sql`ALTER TABLE devkit_orchestration_chat_messages ADD COLUMN IF NOT EXISTS actions_json LONGTEXT NOT NULL DEFAULT '[]' AFTER body`.execute(
    database
  );
