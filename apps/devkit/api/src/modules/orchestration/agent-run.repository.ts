...
export class AgentRunRepository {
  ...
  async recover(uuid: string, actorId: string, threadId: string, turnId: string) {
    await this.database
      .updateTable("devkit_agent_approvals")
      .set({
        decided_at: new Date(),
        decision: "decline",
        status: "cancelled"
  ...
  async startVerification(uuid: string, actorId: string) {
    ...
    const row = await this.database
      .selectFrom("devkit_agent_verifications")
  ...
  async recordVerification(
    ...
    await this.database
      .insertInto("devkit_agent_verifications")
      .values({
  ...
  async activity(uuid: string, actorId: string, label: string) {
    ...
    await this.database
      .insertInto("devkit_agent_run_steps")
      .values({
        completed_at: null,
        kind: "codex.activity",
        label,
    ...
    if (isToolActivity(label)) {
      await this.database
        .insertInto("devkit_agent_tool_calls")
        .values({
          completed_at: null,
          input_json: "{}",
          output_json: "{}",
  ...
  async files(uuid: string, actorId: string, files: string[]) {
    for (const path of files) {
      await this.database
        .insertInto("devkit_agent_artifacts")
        .ignore()
        .values({
  ...
  async requestApproval(
    ...
    await this.database
      .insertInto("devkit_agent_approvals")
      .ignore()
      .values({
        actor_id: actorId,
        decision: null,
  ...
  async resolveApproval(
    ...
    const approval = await this.database
      .selectFrom("devkit_agent_approvals")
  ...
  async find(uuid: string, actorId: string) {
    ...
    const [steps, events, approvals, artifacts, toolCalls, verifications] = await Promise.all([
      this.database
        .selectFrom("devkit_agent_run_steps")
        .selectAll()
        .where("run_uuid", "=", uuid)
        .orderBy("sequence_no")
        .execute(),
      this.database
        .selectFrom("devkit_agent_events")
        .selectAll()
        .where("run_uuid", "=", uuid)
        .orderBy("created_at")
        .execute(),
      this.database
        ...
        .execute(),
      this.database
        .selectFrom("devkit_agent_artifacts")
        .selectAll()
        .where("run_uuid", "=", uuid)
        .orderBy("created_at")
        .execute(),
      this.database
        .selectFrom("devkit_agent_tool_calls")
        .selectAll()
        .where("run_uuid", "=", uuid)
        .orderBy("created_at")
        .execute(),
  ...
  private event(runUuid: string, actorId: string, type: string, payload: unknown) {
    return this.database
      .insertInto("devkit_agent_events")
      .values({
        actor_id: actorId,
        event_type: type,
  ...
  private finishSteps(runUuid: string, status: string) {
    return this.database
      .updateTable("devkit_agent_run_steps")
