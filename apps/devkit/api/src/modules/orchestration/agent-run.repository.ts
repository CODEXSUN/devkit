...
import { assertAgentRunTransition, isTerminalAgentRunStatus, type AgentRunStatus } from "./agent-run.state.js";
...
export class AgentRunRepository {
  ...
  async markWorkspaceCleaned(uuid: string, actorId: string) {
    await this.updateOwned(uuid, actorId, { workspace_cleaned_at: new Date(), workspace_status: "cleaned" });
  ...
  async startVerification(uuid: string, actorId: string) {
    const run = await this.requireRun(uuid, actorId);
    const row = await this.database.selectFrom("devkit_agent_verifications")
  ...
  async recordVerification(uuid: string, input: {
    args: string[];
    attempt: number;
    command: string;
    commandId: string;
    ...
    await this.database.insertInto("devkit_agent_verifications").values({
  ...
  async finishVerification(uuid: string, actorId: string, passed: boolean, attempt: number, fingerprint: string | null) {
  ...
  async requestRework(uuid: string, actorId: string, note: string) {
    ...
    if (!isTerminalAgentRunStatus(run.status)) {
      throw new AppError({ code: "AGENT_RUN_ACTIVE", message: "Wait for the Agent run to finish before review.", statusCode: 409 });
    }
  ...
  async activity(uuid: string, actorId: string, label: string) {
    ...
    await this.database.insertInto("devkit_agent_run_steps").values({
      completed_at: null,
      kind: "codex.activity",
      label,
      output_json: "{}",
    ...
    if (isToolActivity(label)) {
      await this.database.insertInto("devkit_agent_tool_calls").values({
        completed_at: null,
        input_json: "{}",
        output_json: "{}",
  ...
  async files(uuid: string, actorId: string, files: string[]) {
    for (const path of files) {
      await this.database.insertInto("devkit_agent_artifacts").ignore().values({
  ...
  async requestApproval(uuid: string, actorId: string, input: { reason: string; requestId: number; threadId: string }) {
    await this.database.insertInto("devkit_agent_approvals").ignore().values({
      actor_id: actorId,
      decision: null,
      decided_at: null,
      reason: input.reason,
  ...
  async resolveApproval(actorId: string, input: { decision: string; requestId: number; threadId: string }) {
    const approval = await this.database.selectFrom("devkit_agent_approvals")
    ...
    }).where("uuid", "=", approval.uuid).where("actor_id", "=", actorId).executeTakeFirst();
    await this.updateOwned(approval.run_uuid, actorId, { status: input.decision === "decline" ? "cancelled" : "running" });
    await this.event(approval.run_uuid, actorId, "run.approval.decided", { decision: input.decision });
  ...
  async fail(uuid: string, actorId: string, message: string) {
    ...
    await this.updateOwned(uuid, actorId, { completed_at: new Date(), error_message: message, status: "failed" });
    await this.event(uuid, actorId, "run.failed", { message });
  }

  async list(projectUuid: string, actorId: string) {
    const rows = await this.database.selectFrom("devkit_agent_runs").selectAll()
      .where("actor_id", "=", actorId).where("project_uuid", "=", projectUuid)
      .orderBy("updated_at", "desc").limit(30).execute();
  ...
  async find(uuid: string, actorId: string) {
    ...
    const [steps, events, approvals, artifacts, toolCalls, verifications] = await Promise.all([
      this.database.selectFrom("devkit_agent_run_steps").selectAll().where("run_uuid", "=", uuid).orderBy("sequence_no").execute(),
      this.database.selectFrom("devkit_agent_events").selectAll().where("run_uuid", "=", uuid).orderBy("created_at").execute(),
      this.database.selectFrom("devkit_agent_approvals").selectAll().where("run_uuid", "=", uuid).orderBy("created_at").execute(),
      this.database.selectFrom("devkit_agent_artifacts").selectAll().where("run_uuid", "=", uuid).orderBy("created_at").execute(),
      this.database.selectFrom("devkit_agent_tool_calls").selectAll().where("run_uuid", "=", uuid).orderBy("created_at").execute(),
      this.database.selectFrom("devkit_agent_verifications").selectAll().where("run_uuid", "=", uuid).orderBy("attempt_no").orderBy("created_at").execute()
    ]);
    return {
      ...mapRun(run),
      approvals: approvals.map((item) => ({
        createdAt: iso(item.created_at), decision: item.decision, reason: item.reason,
        requestId: item.request_id, status: item.status, uuid: item.uuid
      })),
      artifacts: artifacts.map((item) => ({
        createdAt: iso(item.created_at), label: item.label, path: item.path,
        type: item.artifact_type, uuid: item.uuid
      })),
      events: events.map((item) => ({
        createdAt: iso(item.created_at), payload: parseJson(item.payload_json), type: item.event_type, uuid: item.uuid
      })),
      steps: steps.map((item) => ({
        completedAt: iso(item.completed_at), kind: item.kind, label: item.label,
        sequence: item.sequence_no, startedAt: iso(item.started_at), status: item.status, uuid: item.uuid
      })),
      toolCalls: toolCalls.map((item) => ({
        completedAt: iso(item.completed_at), name: item.tool_name, risk: item.risk_level,
        startedAt: iso(item.started_at), status: item.status, uuid: item.uuid
      })),
      verifications: verifications.map((item) => ({
        args: parseJson(item.args_json), attempt: item.attempt_no, command: item.command_name,
        commandId: item.command_id, completedAt: iso(item.completed_at), durationMs: item.duration_ms,
        exitCode: item.exit_code, label: item.label, required: Boolean(item.required_gate),
        status: item.status, stderr: item.stderr_text, stdout: item.stdout_text, uuid: item.uuid
      }))
    };
  }
  ...
  private event(runUuid: string, actorId: string, type: string, payload: unknown) {
    return this.database.insertInto("devkit_agent_events").values({
      actor_id: actorId, event_type: type, payload_json: JSON.stringify(payload), run_uuid: runUuid, uuid: id()
    }).executeTakeFirstOrThrow();
  }

  private async nextSequence(runUuid: string) {
    const row = await this.database.selectFrom("devkit_agent_run_steps")
      .select(({ fn }) => fn.max<number>("sequence_no").as("maximum"))
      .where("run_uuid", "=", runUuid).executeTakeFirst();
    return Number(row?.maximum ?? 0) + 1;
  }

  private finishSteps(runUuid: string, status: string) {
    return this.database.updateTable("devkit_agent_run_steps").set({ completed_at: new Date(), status })
      .where("run_uuid", "=", runUuid).where("status", "=", "running").execute();
  }
  ...
  private async updateOwned(uuid: string, actorId: string, input: RunUpdate) {
    ...
    await this.database.updateTable("devkit_agent_runs").set(input).where("uuid", "=", uuid).where("actor_id", "=", actorId).executeTakeFirst();
  }

  private async requireRun(uuid: string, actorId: string) {
    const run = await this.database.selectFrom("devkit_agent_runs").selectAll()
...
function mapRun(run: Selectable<AgentRunsTable>) {
  return {
    ...
    projectUuid: run.project_uuid, resultSummary: run.result_summary, startedAt: iso(run.started_at), status: run.status,
    updatedAt: iso(run.updated_at), uuid: run.uuid,
    baseRevision: run.base_revision, branchName: run.branch_name, sourceRoot: run.source_root,
    commitHash: run.commit_hash, committedAt: iso(run.committed_at), reviewStatus: run.review_status,
    verificationCompletedAt: iso(run.verification_completed_at), verificationStatus: run.verification_status,
    verificationFingerprint: run.verification_fingerprint,
    workspaceCleanedAt: iso(run.workspace_cleaned_at), workspaceMode: run.workspace_mode,
    workspacePath: run.workspace_path, workspaceStatus: run.workspace_status
  };
}
