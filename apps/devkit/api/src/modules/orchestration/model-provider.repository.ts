...
export class ModelProviderRepository {
  ...
  async list(actorId: string) {
    return this.database
      .selectFrom("devkit_model_provider_connections")
      .selectAll()
  ...
  async save(provider: ModelProviderId, input: ModelProviderInput, actorId: string) {
    ...
    if (existing) {
      await this.database
        .updateTable("devkit_model_provider_connections")
        .set(values)
        .where("uuid", "=", existing.uuid)
        .executeTakeFirst();
    } else {
      await this.database
        .insertInto("devkit_model_provider_connections")
  ...
  async remove(provider: ModelProviderId, actorId: string) {
    await this.database
      .deleteFrom("devkit_model_provider_connections")
      .where("actor_id", "=", actorId)
      .where("provider", "=", provider)
      .executeTakeFirst();
  }
