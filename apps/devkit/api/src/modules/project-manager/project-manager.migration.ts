...
export const projectManagerMigration = {
  ...
  key: "devkit.project-manager.sql.v7"
} as const;
...
export async function migrateProjectManagerModule(database: Kysely<DevkitDatabase>) {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_items (
      ...
      UNIQUE KEY uq_devkit_project_manager_items_uuid (uuid),
      UNIQUE KEY uq_devkit_project_manager_items_kind_key (kind, item_key),
      KEY idx_devkit_project_manager_items_kind_order (kind, sort_order, updated_at)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);

  await sql`
    CREATE TABLE IF NOT EXISTS devkit_repository_connections (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_repository_connections_uuid (uuid),
      UNIQUE KEY uq_devkit_repository_connections_name (name)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);

  await sql`
    ALTER TABLE devkit_project_manager_items
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_registry_platforms (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_registry_groups (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      CONSTRAINT fk_project_manager_groups_platform
        FOREIGN KEY (platform_uuid) REFERENCES devkit_project_manager_registry_platforms (uuid),
      CONSTRAINT fk_project_manager_groups_parent
        FOREIGN KEY (parent_group_uuid) REFERENCES devkit_project_manager_registry_groups (uuid)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);

  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_registry_modules (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      CONSTRAINT fk_project_manager_modules_group
        FOREIGN KEY (group_uuid) REFERENCES devkit_project_manager_registry_groups (uuid),
      CONSTRAINT fk_project_manager_modules_parent
        FOREIGN KEY (parent_module_uuid) REFERENCES devkit_project_manager_registry_modules (uuid)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);

  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_activity (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_project_manager_activity_uuid (uuid),
      KEY idx_devkit_project_manager_activity_record (record_kind, record_uuid, created_at)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);

  await sql`
    CREATE TABLE IF NOT EXISTS devkit_project_manager_attachments (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY uq_devkit_project_manager_attachments_uuid (uuid),
      UNIQUE KEY uq_devkit_project_manager_attachments_storage_key (storage_key),
      KEY idx_devkit_project_manager_attachments_record (record_kind, record_uuid, created_at),
      CONSTRAINT fk_devkit_project_manager_attachments_record
        FOREIGN KEY (record_uuid) REFERENCES devkit_project_manager_items (uuid) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `.execute(database);
...
async function renameProjectManagerTables(database: Kysely<DevkitDatabase>) {
  const tables = [
    ["project_manager_items", "devkit_project_manager_items"],
    ["project_manager_registry_platforms", "devkit_project_manager_registry_platforms"],
    ["project_manager_registry_groups", "devkit_project_manager_registry_groups"],
    ["project_manager_registry_modules", "devkit_project_manager_registry_modules"],
    ["project_manager_activity", "devkit_project_manager_activity"],
    ["project_manager_attachments", "devkit_project_manager_attachments"]
  ] as const;
