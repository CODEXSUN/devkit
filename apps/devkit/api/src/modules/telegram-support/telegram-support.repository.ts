...
export class TelegramSupportRepository {
  ...
  async connection() {
    return this.database.selectFrom("devkit_telegram_connections").selectAll().orderBy("created_at", "desc").executeTakeFirst();
  }
  ...
  async connect(token: string, chatId: string, username: string, displayName: string) {
    const result = await this.database.updateTable("devkit_telegram_connections").set({
  ...
  async disconnect() {
    await this.database.updateTable("devkit_telegram_connections").set({ status: "disconnected", updated_at: new Date() }).where("status", "=", "connected").execute();
  }

  async messages(chatId: string) {
    const rows = await this.database.selectFrom("devkit_telegram_messages").selectAll().where("chat_id", "=", chatId).orderBy("created_at", "asc").limit(200).execute();
  ...
  async addMessage(chatId: string, direction: "inbound" | "outbound", body: string, telegramMessageId?: string) {
    await this.database.insertInto("devkit_telegram_messages").values({
