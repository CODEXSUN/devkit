...
export function ProjectTaskControl({ workspacePath }: { workspacePath: string }) {
  ...
  async function refresh() {
    ...
    if (!isDevKitWorkspace(workspacePath) || saved.length || seededWorkspace.current === workspacePath) return;
    seededWorkspace.current = workspacePath;
    await Promise.all(devKitProjectTasks(available).map((task) => desktopClient.saveProjectTask(task.title, task.instructions, task.schedule, task.agentModel, task.skillPath)));
