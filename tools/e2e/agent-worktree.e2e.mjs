...
const modulePath = resolve(
  root,
  "dist/devkit/api/modules/orchestration/agent-worktree.service.js"
);
const budgetModulePath = resolve(
  root,
  "dist/devkit/api/modules/orchestration/agent-run.budget.js"
);
const verificationModulePath = resolve(
  root,
  "dist/devkit/api/modules/orchestration/agent-verification.service.js"
);
const integrationModulePath = resolve(
  root,
  "dist/devkit/api/modules/orchestration/agent-integration.service.js"
);
const fingerprintModulePath = resolve(
  root,
  "dist/devkit/api/modules/orchestration/agent-workspace-fingerprint.js"
);
const temporaryRoot = await mkdtemp(join(tmpdir(), "devkit-agent-worktree-"));
...
try {
  ...
  process.env.DEVKIT_AGENT_ALLOWED_ROOTS = temporaryRoot;
  process.env.DEVKIT_AGENT_WORKTREE_ROOT = worktrees;
  ...
  const unavailableCommand = await runner.run(workspace.path, {
    args: [],
    command: "devkit-command-that-does-not-exist",
    id: "missing",
